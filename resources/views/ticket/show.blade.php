...
{{-- <x-app-layout>
    <div class="min-h-screen flex flex-col sm:justify-center items-center pt-6 sm:pt-0 bg-grey-100 dark:bg-grey-900">
        <h1 class="text-white text-lg font-bold">{{ $ticket->title }}</h1>
        <div class="w-full sm:max-w-xl mt-6 px-6 py-4 bg-white dark:bg-grey-800 shadow-md overflow-hidden sm:rounded-lg">
            <div class="text-white flex justify-between py-4">
                ...
                <p>{{ $ticket->created_at->diffForHumans() }}</p>
                @if ($ticket->attachment)
                    <a href="{{ 'storage/' . $ticket->attachment }}" target="_blank" class="text-blue-500">Attachment</a>
                @endif
            </div>
            <x-primary-button>Edit</x-primary-button>
            <x-primary-button>Delete</x-primary-button>
